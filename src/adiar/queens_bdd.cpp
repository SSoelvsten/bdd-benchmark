...
adiar::bdd queens_S(adiar_bdd_adapter &/*mgr*/, int i, int j)
{
  ...
  int row = N - 1;
  adiar::ptr_t next = adiar::create_terminal_ptr(true);
    ...
    if (row_diff == 0) {
      ...
      do {
        ...
        if (column == j) {
          ...
          adiar::node_t queen = adiar::create_node(label, 0, adiar::create_terminal_ptr(false), next);
        ...
        adiar::node_t out_node = adiar::create_node(label, 0, next, adiar::create_terminal_ptr(false));
